        ...
        <ul class="flex-1 overflow-y-auto">
          <%= for room <- @rooms do %>
            <li class={"mb-1 px-2 py-2 rounded cursor-pointer hover:bg-blue-100 " <> if @selected_room && @selected_room.id == room.id, do: "bg-blue-100 font-bold", else: ""}>
              <button phx-click="select_room" phx-value-room_id={room.id} class="w-full text-left flex flex-col gap-0.5 relative">
                ...
                <%= if unread_count(room) > 0 do %>
                  <span class="absolute right-2 top-2 bg-red-500 text-white text-xs rounded-full px-2 py-0.5"> <%= unread_count(room) %> </span>
                <% end %>
              </button>
            </li>
          <% end %>
      ...
      <div class="flex-1 flex flex-col min-h-[80vh]">
        ...
        <div class="flex-1 flex flex-col justify-end">
          <div class="overflow-y-auto flex-1 px-2 pb-2" id="chat-messages">
            <%= for msg <- @messages do %>
              <div class={if msg.sender_type == "admin", do: "flex justify-end", else: "flex justify-start"}>
                <span class={[(if msg.sender_type == "admin", do: "bg-green-200 text-gray-900", else: "bg-gray-200 text-gray-900"), "inline-block rounded px-3 py-2 my-1 max-w-[70%] text-sm shadow"]}>
                  <%= msg.body %>
                </span>
              </div>
            <% end %>
          ...
          <%= if @selected_room do %>
            <form phx-submit="send_message" class="flex gap-2 border-t pt-2 bg-white sticky bottom-0 z-10">
              <input type="text" name="message" value={@message} placeholder="Type your message..." class="flex-1 rounded border px-2 py-2" autocomplete="off"/>
